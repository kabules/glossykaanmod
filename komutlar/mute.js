...
module.exports.run = async (bot, message, args, member) => {
 ...
 if (!message.member.roles.cache.get(yetkili)) return message.channel.send(embed1) //acebots  
  ...
  if (!mutekisi) return message.channel.send(new Discord.MessageEmbed().setDescription(`<a:AterRed:827572447506858005> Lütfen bir kullanıcı etiketleyiniz!`));
  let sebep = args.splice(2, args.length).join(" ");
  let mutezaman = args[1] 
    if (!mutezaman) return message.channel.send(new Discord.MessageEmbed().setDescription(`<a:AterRed:827572447506858005> Lütfen bir zaman giriniz! \n 1 Saniye = 1s \n 1 Dakika = 1m \n 1 Saat = 1h \n 1 Gün = 1d`));
